...
import { Brain, Plus, Trash2, Save, X, Filter, Globe, FolderOpen } from "lucide-react";
...
export function MemoryPage() {
  const { selectedProjectId, setSelectedProject, projects } = useAppStore();
  ...
  // 当前模式下传给 API 的 projectId
  const activeProjectId = mode === "project" ? (selectedProjectId ?? null) : null;

  async function load() {
    setLoading(true);
    try {
      const data = await api.listMemories(activeProjectId);
      setEntries(data);
    } finally {
      setLoading(false);
    }
  ...
  async function handleSave() {
    ...
    setSaveStatus("idle");
    try {
      await api.writeMemory(fileName, editContent, activeProjectId);
      setSaveStatus("saved");
  ...
  async function handleDelete(entry: MemoryEntry) {
    ...
    await api.deleteMemory(entry.file_name, activeProjectId);
    setSelected(null);
    setIsNew(false);
    await load();
  }
  ...
  // 底部路径显示
  const dirPath = mode === "global"
    ? "~/.claude/memory"
    : selectedProject
      ? `~/.claude/projects/${selectedProject.id}/memory`
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      >
        ...
        <button
          onClick={handleNew}
          disabled={mode === "project" && !selectedProject}
          ...
          {isNew || selected ? (
            <>
              ...
              >
                ...
                {selected && !isNew && selected.file_name !== "MEMORY.md" && (
                  <button
              ...
              >
                ...
                {isNew
                  ? `${dirPath}/${newFileName}`
                  : selected?.file_name === "MEMORY.md"
                    ? "~/.claude/MEMORY.md"
                    : `${dirPath}/${selected?.file_name}`}
              </span>
                <span className="flex-1" />
                {isDirty && <span style={{ color: "var(--accent)" }}>未保存</span>}
            ...
            <div className="flex flex-col items-center justify-center h-full gap-3">
              ...
              <button
                onClick={handleNew}
                disabled={mode === "project" && !selectedProject}
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium mt-2 disabled:opacity-40"
                style={{ background: "var(--accent)", color: "white" }}
              >
